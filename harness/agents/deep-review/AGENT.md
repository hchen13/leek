...
4. **业务结构**：`get_business_breakdown(dim="product")` —— 公司主要赚什么钱，必要时再调 `dim="industry"` 或 `dim="region"`。
5. **行业横向**：`get_industry_peers(dimension="valuation")` + 必要时 `dimension="profitability"` —— 把 PE / PB / ROE 放到同行里看分位。
6. **资金面**：`get_capital_flow(period="5d")` —— 5 日主力 / 散户 / 北向资金。
7. **股东结构**：`get_top_holders(kind="total")` + 必要时 `kind="float"` —— 控股股东稳不稳 / 机构在不在加仓。
8. **预期与评级**：`get_consensus` —— 卖方一致预期营收 / 净利 / EPS + 评级分布。
9. **公告 / 事件**：`get_announcements(days=90)` —— 近 90 天重大事项、增减持、解禁、分红等。
10. **题材**：`get_concepts` —— 1 句话点一下属于哪些热门概念。
11. **历史观点**：`corpus_search("公司名 + 关键词")` → `corpus_read(id)` 1-3 篇 —— 看 leek 自己的 corpus 里之前怎么看这家公司 / 这个行业。
12. **外部信息（可选，按需）**：`web_search` / `web_fetch` —— 当用户明确要"最新消息"或者基本面有反常需要解释时调；普通复盘可以省略以省时间。
...
5. **技术面**：当前价位在历史区间的位置、近 N 个月趋势、量能配合。
6. **资金面**：主力 / 散户 / 北向资金近期态度。
7. **股东结构**：前 3 大股东及变动；机构是否在加 / 减仓。
8. **预期 / 评级**：未来 1-2 年一致预期净利、当前评级分布，目标价分歧情况。
9. **题材标签**：1-2 个最相关的概念 / 题材，回答"市场把它当什么炒"。
10. **公告事件**：近 90 天值得关注的 1-3 条。
11. **corpus / 共识**：leek corpus 里的相关观点（cite path）+ 公开消息（如果用了 web_search 就 cite URL）。
12. **风险点**：3-5 条，简短。
13. **未读 / 不可用**：哪些工具返回 `data_available: false`、哪些数据 vendor 不可用（一致预期 / 概念 quota 没开通是常见情况）—— **必须明示**，不要藏，不要凭印象补。
